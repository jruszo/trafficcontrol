...
checkGroveEnvironment() {
	...
	GROVE_DIR='' GROVE_VERSION='' PACKAGE='' debbuild='' DIST='' DEB=''
	...
	export GROVE_DIR GROVE_VERSION PACKAGE BUILD_NUMBER debbuild DIST DEB GOOS deb_TARGET_OS
...
builddebGrove() {
	...
	# tar
	tar -cvzf "${debbuild}/SOURCES/grove-${GROVE_VERSION}.tgz" grove conf/grove.cfg build/grove.init build/grove.logrotate || { echo "Failed to create archive for debbuild: $?" >&2; return 1; }
