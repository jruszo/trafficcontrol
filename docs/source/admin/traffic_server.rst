...
#. Build the Traffic Server DEB. The best way to do this is to follow the `Apache Traffic Server documentation <https://docs.trafficserver.apache.org/en/7.1.x/getting-started/index.en.html#installation>`_.

#. Build the astats DEB using the appropriate version number - ours are built `here <https://github.com/jruszo/trafficcontrol/tree/master/traffic_server>`_
...
#. Install Traffic Server and astats
	...
	.. code-block:: bash
		:caption: Apache Traffic Server Installation Using :manpage:`apt(8)`

		apt -y install trafficserver-*.deb astats_over_http*.deb
...
The app ``atstccfg`` is installed by the cache config DEB alongside ``t3c``. This app is a helper to ``t3c`` and does not have a stable interface, and should never need to be called directly by Traffic Control administrators.
...
#. Build the DEB by following the instructions in :ref:`dev-building` and install it with :manpage:`deb(8)` or :manpage:`apt(8)`.
#. Install modules required by the DEB if needed

#. For initial configuration or when major changes (like a :term:`Profile` change) need to be made, run the script in "badass" mode. All required DEB packages will be installed, all :abbr:`ATS (Apache Traffic Server)` configuration files will be fetched and installed, and (if needed) the :abbr:`ATS (Apache Traffic Server)` service will be restarted.
