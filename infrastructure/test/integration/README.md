...
To run the integration tests, set up Traffic Ops in Docker (see https://github.com/jruszo/trafficcontrol/tree/master/infrastructure/docker/README.md), and run `run-docker-integration-test.sh`.
...
* Query any other components at their IPs, which can be queried from Traffic Ops at `https://localhost/api/4.0/servers`
  * If you're not using a Traffic Ops client, you'll need to send a login cookie; for an example of how to get the cookie, see https://github.com/jruszo/trafficcontrol/tree/master/infrastructure/docker/traffic_ops/run.sh
